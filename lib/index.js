      ...
      if (isLocal) {
        ...
        if (fileExtension) {
          if (fileExtension.match('(ya?ml|toml|json)')) {
            ...
            if (fileExtension === '.toml') {
              metadata = tomlToJSON(metadata);
            }

            // to temp meta object
            allMetadata[optionKey] = metadata;
        ...
        if (fileExtension) {
          if (fileExtension.match('(ya?ml|toml|json)')) {
            ...
            const filePath = path.join(metalsmith.directory(), key);
          ...
          const directoryPath = path.join(metalsmith.directory(), key);
