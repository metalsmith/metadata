      ...
      if (!srcPath.startsWith('..')) {
        ...
        if (extname(filepath)) {
          const fileread = readFile(absPath)
            ...
            .catch(() => done(new Error('No matching file found for entry "' + relpath(filepath) + '"')))
    ...
    Promise.all(dirPromises)
      .then((filelists) => {
        filelists.forEach((filelist) => {
          const matches = metalsmith.match(extglob, filelist.map((f) => f.path))
          filePromises.push(
      ...
      .then((allFiles) => {
        ...
        allFiles.forEach(({ key, file, path }) => {
          ...
          const newMeta = {}, keypath = key.split('.')
          let current = newMeta
